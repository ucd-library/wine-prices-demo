...
/**
 * Fetch all price observations for a specific wine across catalogs.
 * Matches entries by case-insensitive wine name + producer (null producer
 * matches null). Only entries with a bottle price are returned. The catalog
 * year is parsed from the item's date text (first 4-digit run).
 * @param {object} opts
 * @param {string} opts.wineName
 * @param {string|null} [opts.producer]
 * @returns {Promise<object[]>}
 */
export async function getPriceHistory(opts = {}) {
  const { wineName, producer = null } = opts;
  const result = await query(
    `SELECT we.id, we.price, we.case_price, we.bottle_size, we.vintage_year,
            we.confidence, we.currency,
            i.ark, i.title, i.date AS catalog_date,
            (substring(i.date from '\\d{4}'))::int AS catalog_year,
            p.page_number, p.filename
     FROM wine_entries we
     JOIN items i ON i.id = we.item_id
     JOIN pages p ON p.id = we.page_id
     WHERE LOWER(TRIM(we.wine_name)) = LOWER(TRIM($1))
       AND LOWER(TRIM(we.producer)) IS NOT DISTINCT FROM LOWER(TRIM($2))
       AND we.price IS NOT NULL
     ORDER BY catalog_year NULLS LAST, we.vintage_year NULLS LAST, we.id`,
    [wineName, producer]
  );
  return result.rows;
}
